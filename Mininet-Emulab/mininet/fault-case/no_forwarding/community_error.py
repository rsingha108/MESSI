...
for i in range(0, numOfAS):
    ...
    for bgpNetwork in bgp_network_list:
        ...
        switch_list[edgeRouter].addRoutingConfig(None, "ip prefix-list net_in_as permit " + bgpNetwork)
...
for i in range(0, numOfAS):
    ...
    for j in range(0, sizeOfAS - 1):
        ...
        switch_list[edgeRouter].addRoutingConfig(None, "ip prefix-list net_in_as permit " + snet_list[snet_counter].getNetworkPrefix())
...
switch_list[0].addRoutingConfig(None, "ip prefix-list net_in_as permit " + snet_list[snet_counter].getNetworkPrefix())
snet_counter += 1
adminIP = ip2.split("/")[0]
...
for i in range(0, numOfAS):
    ...
    switch_list[edgeRouter].addRoutingConfig(None, "route-map rmap_in permit 10")
    switch_list[edgeRouter].addRoutingConfig(None, "match community ext_as")
    for j in range(0, numOfAS):
        if j != i:
            switch_list[edgeRouter].addRoutingConfig("bgpd", "bgp community-list standard ext_as permit {}:10".format(j))
