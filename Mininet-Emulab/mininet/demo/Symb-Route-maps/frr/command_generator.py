def cmd_gen(d1,d2,cs,sw,cnsl,rmap,route):
    ...
    with open('generated_commands.txt','w') as g:
        ...
        if rmap["Community"] != "None":
            g.write(f's2{cs} vtysh -c "conf t" -c "bgp community-list 100 {PD[rmap["Community-permit"]]} {rmap["Community-regex"]}"\n')

        if rmap["DeleteCommunity"] != "None":
            g.write(f's2{cs} vtysh -c "conf t" -c "bgp community-list 101 permit {rmap["DeleteCommunity"]}"\n')
        ...
        if rmap["ASPath"] != "None":
            g.write(f's2{cs} vtysh -c "conf t" -c "bgp as-path access-list 99 {PD[rmap["ASPath-permit"]]} {rmap["ASPath-regex"]}"\n')
        ...
        if rmap["DeleteCommunity"] != "None":
            g.write(f' -c "set comm-list 101 delete"')
    ...
    with open('generated_commands.txt','a') as g:
        ...
        for diff in diffs:
            if diff == "Prefix0":
                ...
                if rmap["Community"] == "None":
                    g.write(f's2{cs} vtysh -c "conf t" -c "no bgp community-list 100"\n')
                else:
                    g.write(f's2{cs} vtysh -c "conf t" -c "bgp community-list 100 seq 5 {PD[rmap["Community-permit"]]} {rmap["Community-regex"]}"\n')
            elif diff == "Delcom-list":
                if rmap["DeleteCommunity"] == "None":
                    g.write(f's2{cs} vtysh -c "conf t" -c "no bgp community-list 101"\n')
                else:
                    g.write(f's2{cs} vtysh -c "conf t" -c "bgp community-list 101 seq 5 permit {rmap["DeleteCommunity"]}"\n')
            elif diff == "ASPath-list":
                if rmap["ASPath"] == "None":
                    g.write(f's2{cs} vtysh -c "conf t" -c "no bgp as-path access-list 99"\n')
                else:
                    g.write(f's2{cs} vtysh -c "conf t" -c "bgp as-path access-list 99 seq 5 {PD[rmap["ASPath-permit"]]} {rmap["ASPath-regex"]}"\n')
            elif diff == "Route-map":
                g.write(f's2{cs} vtysh -c "conf t" -c "route-map RMap {PD[rmap["RmapPD"]]} 10"')
                if rmap["Prefix"] != "None":
                    g.write(f' -c "match ip address prefix-list PFXL"')
