...
namespace regex{
    public class RegexGenerator{
        ...
        public Tuple<List<string>,List<string>,string> GeneratePosNegExamples(string com){

            // string regex = "(0|[1-9][0-9]{0,3}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]|6553[0-5])"; // all numbers from 0-65535 accecpted
            // all regexes from 0 to 99
            string regex = "(0|[1-9][0-9]{0,1})"; // all numbers from 0-99 accecpted
            regex = "(" + regex + ":" + regex + ")"; // now it should accept single communities of the form AA:NN
            Regex<char> r1 = Regex.Parse("^" + regex + "$"); // valid community regex of the form "AA:NN" (single)
            ...
            Console.WriteLine("pos ex generated...");
            Console.WriteLine("Number of all +ve examples: " + string.Join("\n",multi_pos.Count)); // multiple community AA:NN AA:NN
            // Console.WriteLine("3 +ve examples: " + string.Join("|",multi_pos.GetRange(0,3)));
            Console.WriteLine("all +ve examples: " + string.Join("|",multi_pos));
            // neg community
            r2 = Regex.Negation(r2);
            r = Regex.Intersect(r1, r2);
            CreateGraph(r);
            NodeCover();
            EdgeCover();
            var all_neg = GenerateExamples();
            Console.WriteLine("neg ex generated...");
            Console.WriteLine("Number of all -ve examples: " + string.Join("\n",all_neg.Count)); 
            // Console.WriteLine("3 -ve examples: " + string.Join("|",all_neg.GetRange(0,3)));
            Console.WriteLine("all -ve examples: " + string.Join("|",all_neg));
