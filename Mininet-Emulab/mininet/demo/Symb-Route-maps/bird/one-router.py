...
while True:
	...
	if decision["NextHop"] == "0":
		decision["NextHop"] = route["NextHop"]
	...
	if (rmap["Set"] != "None") and (decision["Allowed"] == "True") and router_decision == "True":
		if bird_attr["LP"]!=decision["LP"]:
			match_attr = "no"
		if bird_attr["MED"]!=decision["MED"]:
			match_attr = "no"
		if sorted(decision["Community"].split(' '))!=sorted(bird_attr["Community"].split(' ')):
			match_attr = "no"
		if bird_attr["ASPath"]!=decision["ASPath"]:
			match_attr = "no"
		if bird_attr["NextHop"]!=decision["NextHop"]:
			match_attr = "no"
