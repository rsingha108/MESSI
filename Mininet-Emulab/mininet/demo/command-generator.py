...
def commands_gen(as1,as2,lp1,lp2,asp1,asp2,org1,org2,med1,med2):
	with open('generated_commands.txt','w') as g:
		...
		g.write(f's2 vtysh -c "conf t" -c "router bgp 40000" -c "neighbor 1.1.1.1 remote-as {as1}" -c "neighbor 3.3.3.1 remote-as {as2}" -c "network 1.0.0.0" -c "network 2.0.0.0" -c "network 3.0.0.0"\n')
		...
		g.write('s1 echo "neighbor 1.1.1.2{" >> conf.ini\n')
		g.write('s1 echo -e "\\trouter-id 1.1.1.1;" >> conf.ini\n')
		g.write('s1 echo -e "\\tlocal-address 1.1.1.1;" >> conf.ini\n')
		g.write(f's1 echo -e "\\tlocal-as {as1};" >> conf.ini\n')
		g.write('s1 echo -e "\\tpeer-as 40000;" >> conf.ini\n')
		...
		g.write(f's1 echo -e "\\t\'announce route 100.10.0.0/24 next-hop self local-preference {lp1} as-path {asp1} med {med1} origin {org1}\'," >> example.py\n')
		...
		g.write('s2 vtysh -c "show ip bgp" >> /mnt/out_log.txt\n')
		...
		g.write('s3 echo "neighbor 3.3.3.2{" >> conf.ini\n')
		g.write('s3 echo -e "\\trouter-id 3.3.3.1;" >> conf.ini\n')
		g.write('s3 echo -e "\\tlocal-address 3.3.3.1;" >> conf.ini\n')
		g.write(f's3 echo -e "\\tlocal-as {as2};" >> conf.ini\n')
		g.write('s3 echo -e "\\tpeer-as 40000;" >> conf.ini\n')
		...
		g.write('py time.sleep(10)\n')
		g.write('s2 vtysh -c "show ip bgp" > /mnt/out.txt\n')
		g.write('s2 vtysh -c "show ip bgp" >> /mnt/out_log.txt\n')
